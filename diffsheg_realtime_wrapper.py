...
class Utterance:
    ...
    def __init__(
        ...
        # Tail generation tracking (for natural ending blend)
        self.beyond_utterance_waypoints: List[GestureWaypoint] = []  # Waypoints from tail generation beyond actual utterance (generated from zero-padded audio)
    ...
    def clear(self):
        ...
        with self.waypoints_lock:
            ...
            self.beyond_utterance_waypoints.clear()
...
class DiffSHEGRealtimeWrapper:
    ...
    def _create_natural_ending_blend_window(self, last_waypoint: GestureWaypoint, blend_duration_sec: float) -> WaypointWindow:
        """
        Create a blend window for natural utterance endings using tail generation waypoints beyond utterance.
        
        For natural endings, the blend consists of two parts:
        1. Beyond-utterance portion from tail generation (frames generated from zero-padded audio).
           These frames are generated by the diffusion model responding to silence and
           naturally transition toward neutral.
        2. Additional linear blend from the last beyond-utterance frame to ensure we reach exact neutral.
        
        This approach provides smoother, more natural transitions than abrupt pre-defined blends.
        
        Args:
            last_waypoint: The last executed waypoint (for fallback if no beyond-utterance frames available)
            blend_duration_sec: Duration for the additional blend portion in seconds
            
        Returns:
            WaypointWindow containing the natural ending blend trajectory
        """
        execution_waypoints = []
        
        # Part 1: Use tail beyond-utterance frames if available. No need to lock here assuming it's called in a safe context where the utterance has been locked already
        if self.current_utterance.beyond_utterance_waypoints:
            # We have beyond-utterance frames from tail generation - use them
            # These frames already have proper timestamps and indices, but we need to
            # re-index them for the blend window (starting from 0)
            self.logger.info(f"Natural ending: using {len(self.current_utterance.beyond_utterance_waypoints)} beyond-utterance frames from tail generation")
            
            for i, beyond_wp in enumerate(self.current_utterance.beyond_utterance_waypoints):
                # Create new waypoint with updated index/timestamp for blend
                waypoint = GestureWaypoint(
                    waypoint_index=i,
                    timestamp=i / self.gesture_fps,
                    gesture_data=beyond_wp.gesture_data.copy(),
                    is_for_execution=True
                )
                execution_waypoints.append(waypoint)
            
            # Part 2: Add additional linear blend from last beyond-utterance frame to neutral
            start_pose = self.current_utterance.beyond_utterance_waypoints[-1].gesture_data.copy()
        else:
            # No beyond-utterance frames available - fall back to using last waypoint
            self.logger.warning("Natural ending: no beyond-utterance frames available, using last waypoint for blend")
            start_pose = last_waypoint.gesture_data.copy()
        
        # Calculate additional blend frames
        ...
        self.logger.info(f"Natural ending blend: {start_index} beyond-utterance frames + {num_blend_frames} blend frames = {len(execution_waypoints)} total frames")
        ...
        if self.overlap_len > 0:
            for i in range(self.overlap_len):
                context_waypoint = GestureWaypoint(
                    waypoint_index=len(execution_waypoints) + i,
                    timestamp=(len(execution_waypoints) + i) / self.gesture_fps,
                    gesture_data=self.neutral_position.copy(),
                    is_for_execution=False
                )
                context_waypoints.append(context_waypoint)
    ...
    def add_audio_chunk(self, utterance_id: int, chunk_index: int, audio_data: list, 
        ...
        with self.utterance_lock:
            ...
            if self.current_utterance.utterance_id != utterance_id:
                ...
                ## This is an abrupt cancellation since a new utterance is replacing the current one
                self.stop_current_utterance(will_lock=False, is_natural_ending=False)
    ...
    def stop_current_utterance(self, will_lock: bool, is_natural_ending: bool = False):
        """
        ...
        Args:
            will_lock: Whether to acquire the utterance lock
            is_natural_ending: If True, this is a natural utterance ending (utterance completed).
                              If False, this is an abrupt cancellation/replacement.
                              Natural endings use the tail generation invalid frames + additional blend,
                              while abrupt cancellations use a pre-defined blend from the last waypoint.
        ...
        try:
            ...
            if self.current_utterance.utterance_id != Utterance.PLACE_HOLDER_ID:
                ...
                self.logger.debug(f"Stop utterance {utterance_id} (had {total_samples} samples, natural_ending={is_natural_ending})")
                ...
                self._initiate_blend_to_neutral(self.last_executed_waypoint, is_natural_ending=is_natural_ending)
    ...
    def _initiate_blend_to_neutral(self, last_waypoint: Optional[GestureWaypoint], is_natural_ending: bool = False):
        """
        ...
        Args:
            ...
            is_natural_ending: If True, this is a natural utterance ending. Use tail generation's
                              invalid frames (if available) plus additional blend. If False, use
                              a standard pre-defined blend from the last waypoint.
        """
        with self.playback_state_lock:
            ...
            if last_waypoint is not None and self.blend_to_neutral_duration > 0:
                # Create blend window based on ending type
                if is_natural_ending:
                    # Natural ending: use tail generation frames + additional blend
                    self.blend_window = self._create_natural_ending_blend_window(
                        last_waypoint=last_waypoint,
                        blend_duration_sec=self.blend_to_neutral_duration
                    )
                else:
                    # Abrupt cancellation: use pre-defined blend
                ...
                ending_type = "natural" if is_natural_ending else "abrupt"
                self.logger.info(f"Initiated blend to neutral ({ending_type}, duration={self.blend_to_neutral_duration}s, {len(self.blend_window.execution_waypoints)} frames)")
            else:
            ...
            if current_state == PlaybackState.IDLE:
                ...
                with self.utterance_lock:
                    ...
                    if (
                        ...
                        # This is the natural ending - duration is known and playback has reached it
                        if ( 
                            ...
                            self.logger.info(
                                f"Utterance {self.current_utterance.utterance_id} naturally completed "
                                f"(duration={self.current_utterance.total_duration:.3f}s, "
                                f"elapsed={elapsed_time:.3f}s). Initiating natural ending blend."
                            )
                            self.stop_current_utterance(will_lock=False, is_natural_ending=False) ## Forget about natural blending for now
                        else:
    ...
    def _generation_loop(self):
        ...
        while self.running:
            ...
            if should_generate and len(audio_snapshot_full) > 0:
                ...
                window, beyond_utterance_waypoints = self._generate_gesture_window_from_audio(
                ...
                with self.utterance_lock:
                    ...
                    # Store beyond-utterance waypoints from tail generation for natural ending blend
                    if beyond_utterance_waypoints:
                        self.current_utterance.beyond_utterance_waypoints.extend(beyond_utterance_waypoints)
                        self.logger.info(f"Utterance {utterance_id} stored {len(beyond_utterance_waypoints)} beyond-utterance waypoints for natural ending blend")
    ...
    def _generate_gesture_window_from_audio(
        ...
        execution_waypoints = []
        context_waypoints = []
        beyond_utterance_waypoints = []  # Store beyond-utterance portion for natural ending blend
        ...
        if max_valid_frames is not None and num_execution_frames < self.window_step:
            for i in range(num_execution_frames, self.window_step):
                ...
                waypoint = GestureWaypoint(
                    waypoint_index=frame_index,
                    timestamp=timestamp,
                    gesture_data=outputs_np[i],  # [C,] - single frame from zero-padded region
                    is_for_execution=False  # Not for immediate execution, saved for blend
                )
        ...
        self.logger.debug(
            ...
            + (f" + {len(beyond_utterance_waypoints)} beyond-utterance waypoints (TAIL)" if beyond_utterance_waypoints else "")
        )
        return window, beyond_utterance_waypoints
